...
class FaceScan(QDialog):
    ...
    def __init__(self, parent=None, enrollment_data=None):
        ...
        if self.cap is None:
            ...
            QMessageBox.critical(self, "Camera Error", "No working camera found. Check connections and permissions.")
            return
        ...
        self.processing = False  # ✅ ADD THIS - Prevent multiple captures

    def update_frame(self):
        if self.processing:  # ✅ ADD THIS - Skip if already processing
            return
        ...
        # ✅ ADD THIS - Validate frame immediately
        if frame.size == 0:
            print("Empty frame received")
            return
        
        # ✅ ADD THIS - Ensure frame is contiguous in memory
        ...
        # ✅ IMPORTANT - Store frame with proper copy
    ...
    def capture_and_save(self):
        # ✅ FIXED - Use self.captured_frame instead of reading again
        if self.captured_frame is None or self.enrollment_data is None:
            print("No captured frame available")
            return
        ...
        # ✅ Make a proper copy with correct memory layout
        frame = np.ascontiguousarray(self.captured_frame.copy())
        
        # ✅ Validate frame before processing
        print(f"Frame info - Shape: {frame.shape}, Type: {frame.dtype}, Size: {frame.size}")
        
        if frame.size == 0:
            QMessageBox.warning(self, "Frame Error", "Invalid frame captured. Please try again.")
    ...
    def generate_embedding(self, frame):
        try:
            ...
            print(f"[generate_embedding] Validating frame...")
            ...
            print(f"  - Frame size: {frame.size if frame is not None else 'None'}")
            ...
            if not isinstance(frame, np.ndarray):
                print(f"ERROR: Frame is not numpy array, it's {type(frame)}")
                return None
            
            # Ensure frame has 3 dimensions (height, width, channels)
            if len(frame.shape) != 3:
                print(f"ERROR: Frame has wrong dimensions: {frame.shape}")
                return None
            ...
            # ✅ Ensure frame is C-contiguous (critical for face_recognition)
            if not frame.flags['C_CONTIGUOUS']:
                print("WARNING: Frame is not C-contiguous, fixing...")
                frame = np.ascontiguousarray(frame)
            
            print(f"[generate_embedding] Frame validated successfully")
            print(f"[generate_embedding] Converting BGR to RGB...")

            # Convert BGR to RGB
            rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            
            # ✅ Ensure RGB frame is also valid
            if not rgb_frame.flags['C_CONTIGUOUS']:
                print("WARNING: RGB frame is not C-contiguous, fixing...")
                rgb_frame = np.ascontiguousarray(rgb_frame)
            
            if rgb_frame.dtype != np.uint8:
                print(f"WARNING: RGB frame dtype is {rgb_frame.dtype}, converting to uint8")
                rgb_frame = rgb_frame.astype(np.uint8)
            
            print(f"[generate_embedding] RGB frame info:")
            print(f"  - Shape: {rgb_frame.shape}")
            print(f"  - Dtype: {rgb_frame.dtype}")
            print(f"  - C-contiguous: {rgb_frame.flags['C_CONTIGUOUS']}")
            print(f"[generate_embedding] Calling face_recognition.face_encodings...")

            # Get face encodings (128-dimension vector)
            face_encodings = face_recognition.face_encodings(rgb_frame)

            if len(face_encodings) == 0:
                print("ERROR: No face detected for embedding generation")
                return None

            # Get the first face encoding
            embedding = face_encodings[0]

            print(f"[generate_embedding] SUCCESS! Generated embedding with {len(embedding)} dimensions")
